// components/history/ExportModal.jsx
"use client";
...
export default function ExportModal({
  ...
  const getActivityLabel = () => {
    if (activeTab === "all") return t("allActivities");
  ...
  const handleDownload = async () => {
    ...
    if (!token) {
      setStatus("error");
      setErrorMsg(t("export.authError"));
      return;
    }
    ...
    try {
      ...
      const response = await fetch(`${ENDPOINTS.EXPORT_HISTORY}?${params}`, {
        headers: { Authorization: `Bearer ${token}` },
      });

      if (!response.ok) {
        const text = await response.text();
        // Deteksi timeout atau error lain
        if (response.status === 408 || response.status === 504) {
          throw new Error(t("export.timeoutError"));
        }
        throw new Error(text || t("export.exportError"));
      }
      ...
      link.download = `history-export.${ext}`;
      ...
      setStatus("error");
      setErrorMsg(err.message || t("export.exportError"));
    }
  };

  const FormatCard = ({ value, label, icon: Icon, description }) => (
    <button
      type="button"
    ...
    >
      ...
      <div>
        <div className="font-medium text-[var(--assetra-text)]">{label}</div>
  ...
  // Render status sukses (pakai biru/primary)
  const renderSuccess = () => (
    <div className="flex flex-col items-center py-6 text-center">
      ...
      <div className="mt-3 rounded-lg bg-[var(--assetra-surface)] px-4 py-2 text-sm text-[var(--assetra-text-secondary)]">
        {t("export.fileLabel")} {entries.length} {t("export.records")} —{" "}
        {format.toUpperCase()}
      </div>
      <div className="mt-6 flex flex-wrap gap-3">
        <a
          href={fileUrl}
          download={`history-export.${format === "pdf" ? "pdf" : "xlsx"}`}
  ...
  const renderError = () => (
    <div className="flex flex-col items-center py-6 text-center">
      ...
      <p className="mt-1 text-sm text-[var(--assetra-danger)]">{errorMsg}</p>
      <div className="mt-6 flex flex-wrap gap-3">
        <button
          type="button"
  ...
  const renderLoading = () => (
    <div className="flex flex-col items-center py-8 text-center">
      <Loader2 size={40} className="animate-spin text-[var(--assetra-primary)]" />
        ...
        <div className="mb-5 flex items-start justify-between">
          <div>
            <h2 className="flex items-center gap-2 text-xl font-semibold text-[var(--assetra-text)]">
              <Download size={20} className="text-[var(--assetra-primary)]" />
              {t("export.title")}
            </h2>
            ...
            <div className="mb-6 rounded-xl bg-[var(--assetra-surface)] p-4">
              ...
              <div className="space-y-1.5 text-sm">
                <div className="flex justify-between">
                  ...
                  <span className="font-medium text-[var(--assetra-text)]">
                    {getActivityLabel()}
                  </span>
                </div>
                <div className="flex justify-between">
                ...
                <div className="flex justify-between">
                  ...
                  <span className="font-medium text-[var(--assetra-text)]">
                    {entries.length} {t("export.activities")}
                  </span>
                </div>
              </div>
            </div>
