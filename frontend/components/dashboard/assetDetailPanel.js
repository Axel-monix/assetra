...
export default function AssetDetailPanel({ asset, onClose, onEdit, onDelete }) {
  if (!asset) return null;

  return (
    <aside className="w-80 shrink-0 border-l border-[#272D3D] bg-[#0B0F17] p-5 overflow-y-auto">
      <div className="flex items-center justify-between mb-4">
        <h2 className="text-sm font-semibold">Detail Aset</h2>
      ...
      <div className="flex items-center justify-between mb-1">
        <h3 className="text-base font-semibold text-[#8083FF]">{asset.name}</h3>
        <span className={`rounded-md px-2 py-0.5 text-[11px] font-medium ${statusBadge[asset.status]}`}>
          {asset.status}
        </span>
      </div>
      <p className="mb-5 font-[family-name:var(--font-code)] text-xs text-[#71717A]">{asset.id}</p>

      <div className="grid grid-cols-2 gap-3 mb-5">
        <div className="rounded-lg border border-[#272D3D] p-3">
          <div className="text-[10px] uppercase tracking-wide text-[#71717A] mb-1">Category</div>
          <div className="text-sm">{asset.category}</div>
        </div>
        <div className="rounded-lg border border-[#272D3D] p-3">
          <div className="text-[10px] uppercase tracking-wide text-[#71717A] mb-1">Location</div>
      ...
      {asset.specs && Object.keys(asset.specs).length > 0 && (
        <div className="mb-5">
          <h4 className="text-xs font-semibold text-[#A1A1AA] mb-2">Full Specifications</h4>
          <div className="flex flex-col gap-1.5">
            {Object.entries(asset.specs).map(([key, value]) => (
              <div key={key} className="flex items-center justify-between text-sm">
                <span className="text-[#71717A]">{key}</span>
                <span className="font-[family-name:var(--font-desc)]">{value}</span>
              </div>
            ))}
          </div>
        </div>
      ...
      {asset.treatmentHistory && asset.treatmentHistory.length > 0 && (
        <div className="mb-6">
          <h4 className="text-xs font-semibold text-[#A1A1AA] mb-2">Treatment History</h4>
          <div className="flex flex-col gap-3">
            {asset.treatmentHistory.map((entry, index) => (
              <div key={index} className="flex gap-3">
                <div className="mt-1.5 h-1.5 w-1.5 rounded-full bg-[#8083FF] shrink-0" />
                <div>
                  <p className="text-[11px] text-[#71717A] uppercase tracking-wide">{entry.date}</p>
