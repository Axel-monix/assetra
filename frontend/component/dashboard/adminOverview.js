...
const recentActivity = [
  ...
  { text: "Irsyad Pramugyo menambahkan aset Sony A7 IV", meta: "15m lalu · TR-8820" },
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
        <StatCard icon={<Archive size={20} strokeWidth={1.75} />} label="Total Aset" value={stats.totalAssets.toLocaleString("id-ID")} badgeText="+12 This Month" badgeColor="info" />
        <StatCard icon={<Wrench size={20} strokeWidth={1.75} />} label="Butuh Perawatan" value={stats.needMaintenance} badgeText="Urgent" badgeColor="urgent" />
        <StatCard icon={<AlertIcon />} label="Rusak" value={stats.broken} badgeText="Resiko Tinggi" badgeColor="danger" />
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 mb-4">
        ...
        <div className="rounded-xl border border-[#272D3D] bg-[#131824] p-5">
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-sm font-semibold">Distribusi aset dalam kategori</h2>
            <a href="/manage-assets" className="text-xs text-[#A5A7FF] hover:text-[#8083FF]">
              Lihat Detail
            </a>
          </div>
        ...
        <table className="w-full text-sm">
          ...
          <tbody>
            {attentionAssets.map((asset) => (
              <tr key={asset.id} className="border-b border-[#1D2230] last:border-0">
                <td className="py-3 font-[family-name:var(--font-code)] text-xs text-[#A1A1AA]">{asset.id}</td>
                <td className="py-3">{asset.name}</td>
                <td className="py-3 text-[#A1A1AA]">{asset.category}</td>
                <td className={`py-3 text-xs font-medium ${statusStyles[asset.status]}`}>● {asset.status}</td>
                <td className="py-3">
                  <a href={`/manage-assets?asset=${asset.id}`} className="text-[#A5A7FF] hover:text-[#8083FF]">
                    ↗
                  </a>
                </td>
              </tr>
